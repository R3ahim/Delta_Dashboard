...
function TotalSell12() {
  const [user, loading, error] = useAuthState(auth);
    ...
    const fetchData = async () => {
      try {
        const response = await fetch("https://deltaserverer.onrender.com/totalSell12"); // Assumes time.json is in the public folder
